...
public class Neural_Net {
   ...
   public static void main(String args[]) throws FileNotFoundException{
      ...
      double outputData[][] = new double[][]{{10},{10},{99},{105}}; //should probably get better name for this
   	//sounded maybe something like learningData
   	//readInput()
   ...
   public Neural_Net(double[][] X ,double[][] y){
   	//this vs not this ? dont have to use this, but it is fine.  this is used when you have a param for constructor that has the same name as a instance variable
      this.input = X;
      this.output = y;
   ...
   public void lernen() {
      scaling(input); //might want to consider scaling the training data in the constructor, seems like a better place to do this
      scaling(output);
   	//forms new weights
      int x = 0;
      while(x<100){ //why 100?
   ...
   public double[][] forward(){
   	...
   	//z_2 = xW
      z2 = matrixMult(input,W1); //we could consider making our own data structure for a matrix, or using one that is already implemented
   	//could be more clean this way
    	//a2 = f(z_3)
      a2 = matrixSigmoid(z2);
    	//z3 = a_2 * W_2
      z3 = matrixMult(a2,W2);
   ...
   public static double sigmoidPrime(double pre){
      return Math.pow(Math.E, - pre)/(Math.pow(1 + Math.pow(Math.E,-pre),2));
   }
   ...
   public static double[][] transpose(double[][] matrix){
      ...
      for(int i = 0; i < matrix.length; i++){
         for(int j = 0; j < matrix[0].length; j++){
            transpose[j][i] = untrans[i][j];
         }
      }
      return transpose;
   	//is this good form (i fixed it)
   }
   ...
   public static double[][] handamard(double[][] first, double[][] second){
      if(first.length != second.length){
         System.out.println("This will not work, lengths of matrices not the same length");
      	//break;
      }
